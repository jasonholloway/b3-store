...
import woodpigeon.tsSchema.TsSchemaPlugin
...
class TsSchemaSpec extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  private def toTempFile(src: String) = {
    val file = java.io.File.createTempFile("tsSchema", ".scala")
    file.deleteOnExit()
  ...
  "Plugin" should "run without blowing up" in {
    ...
    compile(sources, Files.createTempDirectory("tsSchema").toString)
  }
  ...
  "Each schematized class" should "be saved to named ts file" in {
    ...
    val outputDir = Files.createTempDirectory("tsSchema").toString
  ...
  "TS Schema" should "contain class header" in {
    ...
    output should include regex """class Weasel \{.*\}"""
  }
  ...
  //now need to test output. But to do this, need to decide how to output anyhting at all!
